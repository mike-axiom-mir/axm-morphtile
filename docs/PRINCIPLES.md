...
With shape, appearance and motion all describable in the same little language (and logic always was), no facet's vocabulary is the engine's list any more — and since v0.3 the *language itself* can be extended from inside a world, so even the words are not the engine's to own.
...
| **The vocabulary is not the engine's** | `word.define`: a world teaches itself a word (name, arguments, body) and every expression in it — shape, painting, motion, logic — can use it at once. Words lean on words; one that leans on itself forever stops being readable rather than spinning. The engine's own 26 words cannot be redefined, an unknown word stays inert, and a word is an ordinary merge unit that rolls back. | a word the world taught itself; words can lean on words; a world cannot redefine the engine's words |
