...
## PROVEN (74/74 tests pass: `test/run.js` 16, `test/depth.js` 11, `test/blocks.js` 10, `test/text.js` 6, `test/instances.js` 6, `test/sleeping.js` 11, `test/recipes.js` 12, `test/licensing.js` 2)
...
- **Material**: flat colour, three procedural patterns, emissive, and `paint` (colour as an expression of position). Painting is per triangle of the compiled mesh, so it follows the shape but has no notion of a surface direction or texture coordinates.
