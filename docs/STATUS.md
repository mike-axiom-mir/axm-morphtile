...
## PROVEN (101/101 tests pass: `test/run.js` 16, `test/depth.js` 11, `test/blocks.js` 10, `test/text.js` 6, `test/instances.js` 6, `test/sleeping.js` 11, `test/recipes.js` 12, `test/motion.js` 4, `test/words.js` 6, `test/kits.js` 5, `test/conformance.js` 7, `test/views.js` 5, `test/licensing.js` 2)
...
- **Views** cover a tile's own panel, not yet the workshop's own frame (tabs, cards and layout are still the workshop's). A view can present other tiles but cannot yet set the order of root tiles or the page around them; `{tile: …}` resolves within the same container or by full path.
